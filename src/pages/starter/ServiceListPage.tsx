...
import { Pencil, Eye, Plus, Trash2, Triangle, AlertTriangle, X } from "lucide-react";
...
const AdminServiceListPage = () => {
  const { services, fetchServices, EditServices } = useServiceStore();

  const { reset, handleSubmit, register } = useForm<ServiceCatalog>();
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredServices?.map((service: ServiceCatalog) => (
                <tr
                  ...
                  // onClick={() => ClickOnServiceRow(service)}
                >
  ...
  >
    <div className="w-[320px] rounded-2xl bg-white shadow-xl">
      ...
      <div className="flex justify-end gap-2 border-t  border-gray-300 px-5 py-4">
        ...
        <button
          onClick={() => {
            console.log("DELETE:", serviceToDelete);
